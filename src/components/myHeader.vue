      ...
      <div class="container position">
        ...
        <ul class="navbar-list">
          <li class="col" v-for="(item,index) in navlist" :key="index">
            <a :href="(item.url.startsWith('/'))?baseUrl+item.url:item.url" :class="{'selected':selected==index}">{{item.name}}
              <!-- <span>{{item.icon}}</span> -->
            </a>
          </li>
        </ul>
    ...
    data () {
      return {
        ...
        navlist: [
          ...
          {
            url: '/article-detect-1.html',
            name: '产品检测'
          },
          {
            url: '/article-examine-1.html',
            name: '专业审图'
          },
          {
            url: '/article-safeTraining-1.html',
            name: '安全培训'
          },
          {
            url: '/article-publicBidding-1.html',
            name: '招标公示'
          },
          {
            url: '/119',
  ...
  #myHeader {
    ...
    .navbar-redLine .container ul{float: right;
        li{float: left;color: #fff;margin-left: 40px;position: relative;
          em{width: 120px;height: 120px;position: absolute;top: 40px;left: -35px;z-index: 10001;}
    ...
    .col {
      ...
      & > a.selected {
      border-bottom: 3px solid #c41335;color: #c41335;font-weight: 600;}
      & > a span {
